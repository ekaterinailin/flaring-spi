...
grav_pert_err = TSI['M_pl']/1047.57/TSI['M_star']*2.*TSI['R_star']**3./(TSI['a']*215.032-TSI['R_star'])**3.*np.sqrt((TSI['M_pl_err']/TSI['M_pl'])**2.+(TSI['M_star_err']/TSI['M_star'])**2.+(3*TSI['a_err']*215.032/(TSI['a']*215.032-TSI['R_star']))**2.+(3*TSI['a']*215.032/(TSI['a']*215.032-TSI['R_star'])*TSI['R_star_err']/TSI['R_star'])**2.)
# grav_pert_err = grav_pert_err.filled(np.nan)
TSI.add_column(Column(grav_pert_err,name = 'grav_pert_err'))
...
for i in range(len(TSI)):
    if TSI['Teff'][i] < 6250.:
        ...
        tau_el_err = 1./tau_inv_el*np.sqrt((2*TSI['M_star_err'][i]/TSI['M_star'][i])**2.+(2*TSI['M_pl_err'][i]/TSI['M_pl'][i])**2.+(6*TSI['a_err'][i]/TSI['a'][i])**2+(6*TSI['R_star_err'][i]/TSI['R_star'][i])**2)
    else:
        #radiative envelope
        tau_inv_el =  (1/(0.25*5*10**9.)*q[i]**2.*(1+q[i])**(5./6.)*(TSI['a'][i]*215.032/TSI['R_star'][i]/6.)**(-17./2.))*(5*10**9.)
        tau_el_err =0.25*5.*10.**9.*(TSI['a'][i]*215.032/TSI['R_star'][i]/6)**(17./2.)*np.sqrt( (17./2.*(q[i]**-2. + q[i]**(-17./6.)))**2. * ((TSI['a_err'][i]/TSI['a'][i])**2. + (TSI['R_star_err'][i]/TSI['R_star'][i])**2.) + 
                                                                                              (2*q[i]**-2. + 17./6.*q[i]**(-17./6.))**2. * ( (TSI['M_star_err'][i]/TSI['M_star'][i])**2. + (TSI['M_pl_err'][i]/TSI['M_pl'][i])**2.))/(5*10**9.)
    tau_inv.append(tau_inv_el)
    tau_err.append(tau_el_err)
...
TSI.add_column(Column(tau_err,name = 'tidal_disip_timescale_err'))
...
torque_conv_err = torque_conv*np.sqrt((5.*TSI['R_star_err']/TSI['R_star'])**2. + (6.*TSI['a_err']/TSI['a'])**2.+ (0.5*TSI['M_star_err']/TSI['M_star']*(1+q**-1.)**-1.)**2. + (TSI['M_pl_err']/TSI['M_pl']*(2.-0.5*(1+q**-1)**-1.))**2.)
...
TSI.add_column(Column(torque_conv_err,name = 'torque_conv_err'))
